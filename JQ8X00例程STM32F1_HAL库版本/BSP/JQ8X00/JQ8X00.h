...
//延时函数重定义
#define JQ8x00_ms(ms)    HAL_Delay(ms)         //JQ8x00延时函数定义，用户自行更改为自己的延时函数
#define JQ8x00_us(us)    delay_us(us)

//串口函数重定义
#define JQ8x00_UART(pointer,len) HAL_UART_Transmit(&huartx,pointer,len,0xffff)        //pointer为指针数据传入数据首地址，len为要发送数据个数
...
//单线串口IO配置
#define JQ8x00_VPP_RCC           __HAL_RCC_GPIOA_CLK_ENABLE        //时钟
...
//忙检测IO配置
#define JQ8x00_BUSY_RCC          __HAL_RCC_GPIOA_CLK_ENABLE       //时钟
#define JQ8x00_BUSY_GPIO         GPIOA
#define JQ8x00_BUSY_GPIO_Pin     GPIO_PIN_0                 //IO

//BUSY电平读取
#define JQ8x00_BUSY_Read  HAL_GPIO_ReadPin(JQ8x00_BUSY_GPIO,JQ8x00_BUSY_GPIO_Pin)		//读取电平
