...
/* PB6=SCL, PB7=SDA，软件 I2C 位延时（168MHz 下约 100kHz） */
#define OLED_I2C_PIN_SCL       GPIO_PIN_6
#define OLED_I2C_PIN_SDA       GPIO_PIN_7
...
{
	HAL_GPIO_WritePin(OLED_I2C_GPIO_PORT, OLED_I2C_PIN_SCL, (GPIO_PinState)BitValue);
	OLED_I2C_Delay();
}
...
{
	HAL_GPIO_WritePin(OLED_I2C_GPIO_PORT, OLED_I2C_PIN_SDA, (GPIO_PinState)BitValue);
	OLED_I2C_Delay();
}
...
void OLED_GPIO_Init(void)
{
	...
	/* 软件 I2C：开漏输出，须与模块板上拉电阻配合（一般 OLED 模组已带 4.7k） */
	...
	GPIO_InitStruct.Pin = OLED_I2C_PIN_SCL;
	HAL_GPIO_Init(OLED_I2C_GPIO_PORT, &GPIO_InitStruct);

	GPIO_InitStruct.Pin = OLED_I2C_PIN_SDA;
	HAL_GPIO_Init(OLED_I2C_GPIO_PORT, &GPIO_InitStruct);
